...
const MIGRATION_DIRECTORIES = ['migrations/tenant/workspace'];
...
const OWNED_SCHEMAS = new Set(['workspace_schema', 'integration_schema']);
const CORE_SCHEMAS = [
  'identity_schema',
  'tenancy_schema',
...
describe('Workspace architecture boundary', () => {
  ...
  it('Platform API và Portal không phụ thuộc package của module', () => {
    expect(readDependencies('apps/api/package.json')).not.toHaveProperty('@enterprise-platform/module-workspace');
    expect(readDependencies('apps/web/package.json')).not.toHaveProperty('@enterprise-platform/feature-workspace');
  });

  it('module không phụ thuộc package của Core hay của module khác', () => {
    const forbidden = Object.keys(readDependencies('packages/modules/workspace/package.json')).filter(
      (name) =>
        name.startsWith('@enterprise-platform/platform-') ||
        (name.startsWith('@enterprise-platform/module-') && name !== '@enterprise-platform/module-workspace'),
    );
