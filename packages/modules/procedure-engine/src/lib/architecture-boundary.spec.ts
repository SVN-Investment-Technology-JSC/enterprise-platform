...
const MIGRATION_DIRECTORIES = [
  'migrations/tenant/procedure',
  'packages/modules/procedure-engine/src/lib/infrastructure/persistence/migrations',
];
...
describe('Procedure Engine architecture boundary', () => {
  ...
  it('Platform API và Portal không phụ thuộc package của module', () => {
    expect(readDependencies('apps/api/package.json')).not.toHaveProperty('@enterprise-platform/module-procedure-engine');
    expect(readDependencies('apps/web/package.json')).not.toHaveProperty('@enterprise-platform/feature-procedure-engine');
  });

  it('module không phụ thuộc package của Core hay của module khác', () => {
    const forbidden = Object.keys(readDependencies('packages/modules/procedure-engine/package.json')).filter(
      (name) =>
        name.startsWith('@enterprise-platform/platform-') ||
        (name.startsWith('@enterprise-platform/module-') && name !== '@enterprise-platform/module-procedure-engine'),
    );
