...
export interface StockBalanceDto { warehouseId:string; warehouseCode:string; itemId:string; itemCode:string; itemName:string; uom:string; onHand:number; reserved:number; available:number; minStock:number }
export interface StockTransactionDto { id:string; code:string; date:string; type:string; itemCode:string; itemName:string; warehouseCode:string; quantity:number; balanceBefore:number; balanceAfter:number; unitCost:number; referenceType:string; referenceId:string; notes?:string }
...
export interface CreateAssetDto { code:string; name:string; parentId?:string; type:'PLANT'|'SYSTEM'|'EQUIPMENT'|'COMPONENT'; criticality?:'CRITICAL'|'HIGH'|'MEDIUM'|'LOW'; serialNumber?:string; specs?:Record<string,unknown> }
...
export interface AssetSummaryDto { id:string; code:string; name:string; parentId?:string; type:'PLANT'|'SYSTEM'|'EQUIPMENT'|'COMPONENT'; status:string; criticality:string; serialNumber?:string; qrCode?:string; specs?:Record<string,unknown>; bomCount:number; bom:AssetBomDto[]; documents?:AssetDocumentDto[]; maintenanceHistory?:MaintenanceEventDto[]; procedures?:MaintenanceProcedureDto[] }
...
export interface StockLineDto { itemId:string; quantity:number; locationId?:string; lotId?:string; unitCost?:number }
export interface ImportStockDto { receiptNo:string; warehouseId:string; supplierCode?:string; lines:StockLineDto[] }
export interface ExportStockDto { issueNo:string; warehouseId:string; referenceType?:string; referenceId?:string; lines:StockLineDto[] }
export interface InventoryWorkspaceDto { warehouses:WarehouseSummaryDto[]; items:ItemDetailDto[]; balances:StockBalanceDto[]; lowStock:StockBalanceDto[]; transactions:StockTransactionDto[]; assets:AssetSummaryDto[]; serials:SerialTrackingDto[]; reservations:ReservationSummaryDto[] }
