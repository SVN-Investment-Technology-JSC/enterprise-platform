...
export function SparePartPanel({
  ...
  /**
   * Danh sách vật tư từ danh mục kho (materials), ưu tiên hiển thị và lọc trừ đi những mã đã có trong BOM.
   */
  ...
  const startEditing = () => {
    setDraftLines(
      (lines ?? []).map((l) => ({
        ...
        isCriticalSpare: l.isCriticalSpare,
      })),
    );
    setIsEditing(true);
  };
  ...
  const handleSaveAll = async () => {
    setSaving(true);
    try {
      // Xoá các dòng cũ không còn trong draft
      const currentIds = lines ?? [];
      for (const line of currentIds) {
        if (!draftLines.some((d) => d.materialCode === line.materialCode)) {
          await removeAssetSparePart(assetCode, line.id);
        }
      }
      // Thêm hoặc cập nhật
      for (const draft of draftLines) {
        const existing = currentIds.find((l) => l.materialCode === draft.materialCode);
        if (!existing) {
          await addAssetSparePart(assetCode, {
            materialCode: draft.materialCode,
            standardQuantity: draft.standardQuantity,
            isCriticalSpare: draft.isCriticalSpare,
          });
        }
      }
      await reload();
        ...
        <div className={styles.inlineEditContainer}>
          ...
          <div className={styles.inlineEditTableWrap}>
            <table className={styles.inlineEditTable} style={{ tableLayout: 'fixed' }}>
              <thead>
                <tr>
                  <th style={{ width: '44%', padding: '8px 10px' }}>Tên vật tư con / Mã</th>
                  <th style={{ width: '26%', padding: '8px 10px' }}>Định mức số lượng</th>
                  <th style={{ width: '18%', padding: '8px 10px' }}>Trọng yếu</th>
                  <th style={{ width: '12%', textAlign: 'center', padding: '8px 6px' }}>Thao tác</th>
                </tr>
              </thead>
              <tbody>
                {draftLines.map((draft, index) => {
                  ...
                  const child = (childMaterials ?? []).find(
                    (c) => c.materialCode === draft.materialCode,
                  );
                  const name = mat?.name ?? child?.materialName ?? draft.materialCode;
                  const unit = mat?.unit ?? child?.unit ?? '';
                  return (
          ...
          <div className={styles.spareForm} style={{ marginTop: '6px', padding: '8px 10px', background: '#ffffff', borderRadius: '6px', border: '1px dashed #cbd5e1', position: 'relative' }}>
                      ...
                      .map((opt) => (
                        <div
                          ...
                          onClick={() => {
                            ...
                            const child = (childMaterials ?? []).find((line) => line.materialCode === opt.code);
            ...
            />
            <label style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', fontSize: '12px', whiteSpace: 'nowrap' }}>
              <input
                type="checkbox"
                checked={critical}
                disabled={disabled}
