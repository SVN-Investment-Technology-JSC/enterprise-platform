...
| DakRoSa | `admin@dakrosa.local` | Giá trị `SEED_TENANT_ADMIN_PASSWORD` |
| An Phát | `admin@anphat.local` | Giá trị `SEED_TENANT_ADMIN_PASSWORD` |
| Minh Long | `admin@minhlong.local` | Giá trị `SEED_TENANT_ADMIN_PASSWORD` |
...
docker compose @compose exec minhlong-db psql -U tenant -d minhlong
```
...
Nếu stack đang chạy và chỉ sửa Procedure Engine:

```powershell
docker compose @compose up --build --wait procedure-api procedure-web
```

Gateway không cần restart; Docker DNS sẽ tiếp tục định tuyến sang container Procedure mới.
...
`down --volumes` xóa toàn bộ Platform DB, ba Tenant DB, RabbitMQ và MinIO data của Compose full-stack. Chỉ dùng khi chắc chắn muốn tạo lại dữ liệu seed từ đầu.
