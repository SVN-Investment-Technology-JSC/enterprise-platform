...
Migrator chỉ khởi tạo Platform Admin. Tenant và dữ liệu nghiệp vụ không tự sinh ra: đăng nhập Platform Admin rồi tạo tenant qua luồng quản trị.
...
`down --volumes` xóa toàn bộ Platform DB, toàn bộ database tenant trong `tenant-db`, RabbitMQ và MinIO data của Compose full-stack. Chỉ dùng khi chắc chắn muốn tạo lại dữ liệu từ đầu.
