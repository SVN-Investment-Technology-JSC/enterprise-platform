...
function updateModuleStatus(
  ...
  return modules.map((module) =>
    module.key === moduleKey ? { ...module, entitlementStatus: status } : module,
  );
}
...
export function TenantEntitlements({
  ...
  async function setEntitlement(module: TenantModuleEntitlement, enabled: boolean) {
    ...
    try {
      ...
      const payload = (await response.json()) as SetTenantEntitlementResponse & ApiErrorPayload;
      if (!response.ok) {
        throw new Error(errorMessage(payload, 'Không thể cập nhật entitlement.'));
      }
      setOverview((current) => ({
        ...current,
        modules: updateModuleStatus(current.modules, module.key, payload.status),
      }));
      setNotice(
        enabled
      ...
      setError(cause instanceof Error ? cause.message : 'Không thể cập nhật entitlement.');
    } finally {
  ...
  return (
    <div className={styles.workspace}>
      <Link className={styles.backLink} href="/platform">← Quay lại danh sách tenant</Link>

      <section className={styles.tenantContext} aria-label="Thông tin tenant">
        <div><small>Tenant</small><strong>{overview.tenant.slug}</strong></div>
        <div><small>Tenant admin</small><strong>{overview.tenant.admin?.email ?? 'Chưa có'}</strong></div>
        <div><small>Dedicated DB</small><strong>{overview.tenant.database?.databaseName ?? 'Chưa cấu hình'}</strong></div>
        <div><small>Secret reference</small><strong><code>{overview.tenant.database?.secretRef ?? '—'}</code></strong></div>
      </section>
      ...
      <header className={styles.sectionHeader}>
        ...
        <p>Worker tự động xử lý migration. Trang sẽ cập nhật trạng thái trong lúc provisioning.</p>
      </header>
          ...
          return (
            <article className={styles.moduleCard} key={module.key}>
              <header>
                ...
                <span className={`${styles.status} ${STATUS_CLASS[module.entitlementStatus]}`}>
                  {STATUS_LABEL[module.entitlementStatus]}
                </span>
              </header>
              <div>
                <small>{module.key} · v{module.version}</small>
                <h3>{module.name}</h3>
                <p>{module.description}</p>
              </div>
              <dl>
                <div><dt>Launch URL</dt><dd><code>{module.launchUrl}</code></dd></div>
                <div><dt>Đã provision</dt><dd>{module.provisionedVersion ? `v${module.provisionedVersion}` : '—'}</dd></div>
              </dl>
              {module.latestJob?.error && failed ? (
                <p className={styles.jobError} role="alert">{module.latestJob.error}</p>
              ) : null}
              <footer>
                <small>{provisioning ? 'Đang tạo/cập nhật schema…' : enabled ? 'Module đang hiển thị trong Tenant Portal.' : 'Dữ liệu cũ không bị xóa khi thu hồi.'}</small>
                <button
                  className={enabled ? styles.disableButton : styles.enableButton}
                  disabled={busy}
