...
function moduleHref(module: TenantModuleCatalogItem, tenantSlug: string) {
  ...
  if (module.key === 'hrm') return `/t/${tenantSlug}/hrm`;
...
function ActiveModuleCard({
  ...
  const href = moduleHref(module, tenantSlug);
  const isInternal = href.startsWith('/');

  const cardContent = (
    <article className="group/card flex min-h-60 flex-col rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition duration-200 hover:-translate-y-0.5 hover:shadow-md cursor-pointer">
      <div className="mb-5 flex items-start justify-between gap-3">
        <div className="grid size-12 place-items-center rounded-xl bg-slate-100 text-[#091426] transition-colors group-hover/card:bg-slate-200">
      ...
      <h3 className="text-base font-semibold text-slate-950 group-hover/card:text-blue-600 transition-colors">
        {module.name}
      </h3>
      ...
      <div className="mt-5 flex items-center justify-between gap-3 border-t border-slate-100 pt-4">
        ...
        <span className="inline-flex items-center gap-1 text-sm font-semibold text-[#091426] group-hover/card:text-blue-600 group-hover/card:underline">
          Mở ứng dụng <ArrowRight className="size-3.5 transition-transform group-hover/card:translate-x-0.5" />
        </span>
      </div>
    </article>
  );
  ...
  return isInternal ? (
    ...
    <a href={href} className="block no-underline" target="_blank" rel="noreferrer">
      {cardContent}
    </a>
  );
}
